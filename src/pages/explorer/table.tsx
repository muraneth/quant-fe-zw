            ...
            return snap ? (
              <div
                ...
                onClick={() => {
                  if (snap.handle_name) {
                    navigate(
                      `/charts?symbol=${baseInfo.symbol}&handle_name=${snap.handle_name}&type=${'independent_line'}`
                    );
                  }
                }}
              >
      ...
      for (const token of userConfig.tokens) {
        try {
          ...
          setTokenDetailList((prev) => {
            if (!prev.some(item => item.base_info.contract_address === result.base_info.contract_address)) {
              return [...prev, result];
            }
            return prev;
          });
